...
def generateAnimat(records: [[float]], start_state: (float, float), end_state: (float, float), 
	...
	fps: int = 1, generate_gif: bool = False, filename: str = 'animat', just_vals: bool = False) -> amp.Animation:
	...
	def animate(i):
		...
		ax.imshow(records[i], interpolation='none', cmap='jet')

		# Put values on p[i]ix map
		for j in range(len(records[i])):
			for k in range(len(records[i][j])):

				text=[str(records[i][j][k])] if not is_last_frame else []
